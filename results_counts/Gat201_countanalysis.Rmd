...
## At 30 mins, 2hrs and 4hrs 15 ml samples were cellected, fix and freeze and put at -80 degrees.
...
## Make a clustered heatmap of count data showing similarity between samples
...
counts[,-(1:6)] %>%
    add(1) %>%
    log10() %>%
    pheatmap(main = "heatmap of counts",show_rownames = FALSE)
```
...
# normalisation: from mutate
# actual count for row /sum of all genes in that sample (sample ID) x 1e6 to give TPM (ratio so units do not matter)
# can do this because our reads are at 3 end.
# for rna seq on other method would have to normalise to lenght of gene as well
...
# very long as for each gene we have info
...
# write as a function so only have to change gene names once
...
## Plot TPMs for two genes: Change genes in code above line 151

```{r}
plot_tpm_somegenes()
```

# Remember to change name to genes from line 151
```{r}
ggsave("~/Documents/GitHub/CryptoGat201RNASeq_draft/results_counts/TPM_Gat201_TSA3.pdf",
       width = 5,height=4)
```

## Reshape the TPMs into a "wide" table that we can use for heatmap
# reshape counts long using pivot_wider
# id_cols is what to call the new columns, Geneid, take names used from SampleID and the values used from TPM
...
Heatmap <- foldchange_wide %>%
    ...
    .[,-1] %>%
    log10() %>%
    pheatmap(main = "Heatmap of FoldChange, genes with min(TPM) > 20",show_rownames = FALSE,
             color = colorRampPalette(c("yellow", "orange","red"))(31),
...
```
The 4 main clusters are slpit by time.1)YO 2)R/RS 30 mins 3) R/RS 120 min, 4) R/RS 240 min

## Principal Component Analysis of fold-changes, for genes with min. TPM 20
```{r}
...
ggsave("~/Documents/GitHub/CryptoGat201RNASeq_draft/results_counts/plot_pca_log10foldchange_mintpm20_PC12.pdf",
       plot_pca_log10foldchange_mintpm20_PC12,width = 4,height=4)
plot_pca_log10foldchange_mintpm20_PC12
```
Blue = Y at 0 mins, Green/Red Top Left = R/RS at 4 hours, Green/Red Bottom Left = R.RS at 2 hours, Green/Red at Bottom Right = R/Rs at 30 mins
...
Red/Green collected at the Left Side = Mix of 30 and 2 hours for all 4 strains with. Slight media separation.
...
37,40,51,54 (Green) = Gat201 B/M RS at 30 mins
