...
DEGAT201_list_anytime_arrange_240 <- 
  deseq_df_CT_vsGAT201 %>%
...
plotheat_log2FC(genes = DEGAT201_list_anytime, 
                  data = deseq_df_CT_vsGAT201 %>%
                    dplyr::filter(gene %in% DEGAT201_list_anytime) %>%
                    dplyr::mutate(gene = factor(
                      gene, 
                      levels = DEGAT201_list_anytime_arrange_240)) 
                ) + 
...
```{r select_DEGAT201_more_240}
DEGAT201_up_anytime2 <- 
  deseq_df_CT_vsGAT201 %>%
  dplyr::filter(log2FC > 3, padj < 0.05) 

DEGAT201_down_anytime2 <- 
  deseq_df_CT_vsGAT201 %>%
  dplyr::filter(log2FC < -3, padj < 0.05)

DEGAT201_list_anytime2 <- c(unique(DEGAT201_up_anytime2$gene),
                           unique(DEGAT201_down_anytime2$gene))

# list genes arranged by log2FC at R_240
DEGAT201_list_anytime2_arrange_240 <- 
  deseq_df_CT_vsGAT201 %>%
  dplyr::filter(gene %in% DEGAT201_list_anytime2, 
                Condition_Time %in% c("R_240", "RS_240")) %>%
  group_by(gene) %>%
  dplyr::summarise(log2FC = mean(log2FC)) %>%
  dplyr::arrange(log2FC) %>%
...
plotheat_log2FC(genes = DEGAT201_list_anytime2, 
                  data = deseq_df_CT_vsGAT201 %>%
                    dplyr::filter(gene %in% DEGAT201_list_anytime) %>%
                    dplyr::mutate(gene = factor(
                      gene, 
